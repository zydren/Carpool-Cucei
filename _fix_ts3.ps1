 ...
 $firstPart = $lines[0..76] -join "`r`n"; 
 $lastPart = $lines[167..($lines.Length-1)] -join "`r`n"; 
 $middlePart = "
  /**
   ...
   *   búsqueda del punto indicado por el pasajero (no se hacen llamadas OSRM en la búsqueda;
   *   la distancia se calcula localmente a partir de los segmentos del LineString).
   */
export type TripMatchType = 'textual' | 'origin' | 'routeNearby';
...
export interface TripSearchResult extends Trip {
    ...
    /**
     * Distancia en km entre el punto de búsqueda y la ruta del viaje
  ...
  /**
   * Distancia mínima (km) desde un punto geográfico hasta una línea
   * representada como un GeoJSON LineString.
   * 
   * Se calcula sobre cada segmento consecutivo del trayecto (par de
   * coordenadas [lng,lat]) y se toma el mínimo. No se hace ninguna
   * petición a OSRM: la ruta ya está guardada en route_geometry y la
   ...
   * 
   * Se proyecta el punto sobre la geodésica del segmento. Si la proyección
...
"; 
  $fullContent = $firstPart + "`r`n" + $middlePart + "`r`n" + $lastPart;
  [IO.File]::WriteAllText("src\\services\\tripService.ts", $fullContent, [Text.Encoding]::UTF8);
  Write-Output "RECONSTRUCTED"; exit